...
use App\Admin\Actions\Updatedriver;
...
class DriverController extends AdminController
{
    ...
    public function availabledrivers(Content $content)
    {
        $data = Driver::where('status',1)
        ->latest()->get();

        return $content->title('Available Driver')
        ->view('admin.drivers',compact('data'));
    }

    public function driverinfo(Content $content, $id)
    {
        $data = Driver::where('id',$id)
        ->first();

        return $content->title('Driver Info')
        ->view('admin.driver-info',compact('data'));
    }
